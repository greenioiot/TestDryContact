...
{
  char TOKEN[40] = "";
  char SERVER[40] = "mqtt.example.com";
  int PORT = 1883;
...
void _initDryContact()
{
  for (int a = 0; a < 4 ; a++)
  {
    pinMode(pinIn[a], INPUT);
    Serial.print("contact"); Serial.print(":");    Serial.println(digitalRead(pinIn[a]));
...
void setup() {
  ...
  WiFiManagerParameter program_param("program", "Program", program, 50);
  ...
  deviceToken = uint64ToString(chipId);
